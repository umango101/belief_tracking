...
ONE_SHOT_CHAT = [
    HumanMessage(
        content="Story: {story}\nQuestion: {question}".format(
            story=ONE_SHOT["story"], question=ONE_SHOT["question"]
        )
    ),
    AIMessage(content="Answer: {answer}".format(answer=ONE_SHOT["answer"])),
]
ONE_SHOT_CHAT_COT = [
    HumanMessage(
        content="Story: {story}\nQuestion: {question}".format(
    ...
    AIMessage(
        content="Thought: {thought}\nAnswer: {answer}".format(
            answer=ONE_SHOT["answer"], thought=ONE_SHOT["thought"]
        )
    ),
]
...
class EvaluateLLM:
    ...
    def __init__(self, llm, tokenizer, method="0shot"):
        self.llm = llm
...
Answer:"""
            ...
            one_shot = "Story: {story}\nQuestion: {question}\nAnswer: {answer}".format(
                story=ONE_SHOT["story"],
                question=ONE_SHOT["question"],
                answer=ONE_SHOT["answer"],
            )
            self.prompt = (
                "{instruction}"
                + "\n"
...
A:"""
    ...
    def predict_answer(self, story, question):
        if "chat" in self.method:
            ...
            prompt += [
                HumanMessage(content=self.prompt.format(story=story, question=question))
            ]
            response = self.llm.generate([prompt])
            response = response.generations[0][0].text
        elif "cot" not in self.method:
            prompt = self.prompt.format(
                instruction=self.instruction, story=story, question=question
            )
