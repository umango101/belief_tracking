...
def train_mask(
    ...
    for l in range(model.config.num_hidden_layers):
        if dataset_func.__name__ == "get_bigtom_value_fetcher_exps":
            sing_vecs[l] = torch.load(
                f"../svd_results/BigToM/last_token/singular_vecs/{l}.pt"
            ).cpu()
            ...
            sing_vecs[l] = torch.load(
                f"../svd_results/BigToM/query_charac_new/singular_vecs/{l}.pt"
            ).cpu()
        elif dataset_func.__name__ == "get_bigtom_visibility_exps":
            sing_vecs[l] = torch.load(
                f"../svd_results/BigToM/visibility_sent/singular_vecs/{l}.pt"
            ).cpu()
    ...
    for layer_idx in layer_range:
        for lamb in lambs:
                ...
                for bi, batch in tqdm(enumerate(train_dataloader), total=len(train_dataloader)):
                    ...
                    with model.trace() as tracer:
                        ...
                        if dataset_func.__name__ in [
                            ...
                            # For Visibility Sentence Variable
                    ...
                    if bi % 4 == 0:
                        ...
                        print(
                            f"Epoch: {epoch}, Batch: {bi}, Task Loss: {task_loss.item():.4f}, "
            ...
            # Testing
            print(f"Testing started for layer: {layer_idx}, lambda: {lamb}")
            correct, total = 0, 0
                ...
                for bi, batch in tqdm(enumerate(test_dataloader), total=len(test_dataloader)):
                    ...
                    with model.session() as session:
                        ...
                        if dataset_func.__name__ in [
                            ...
                            del alt_acts, proj_matrix, masked_vec
                            torch.cuda.empty_cache()
                            ...
                            del alt_layer_out, proj_matrix, masked_vec
                            torch.cuda.empty_cache()
...
def main():
    ...
    print(
        f"Variable: {args.variable}, Layer Range: {args.layer_start}-{args.layer_end}, "
    ...
    elif args.variable == "query_character":
        dataset_func = get_bigtom_query_charac
    else:  # visibility
        dataset_func = get_bigtom_visibility_exps
